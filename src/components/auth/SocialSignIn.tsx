 ...
 *
 * LAYOUT (E020): all three in ONE compact row — the stacked full-width version
 * ate most of the sign-up page. Each button is icon + short label so the row
 * fits without wrapping; the row falls back to a 1-column stack only on very
 * narrow screens.
 *
...
function LinkedInIn({ className = "h-[18px] w-[18px]" }: { className?: string }) {
  return (
...
function AppleMark({ className = "h-[19px] w-[19px]" }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 24 24" fill="currentColor" aria-hidden>
        ...
        {BUTTONS.map((b) => {
          ...
          return (
            <button
              ...
              className={
                "flex items-center justify-center gap-2 rounded-full border px-3 py-2.5 text-[14px] font-bold transition-colors disabled:cursor-not-allowed disabled:opacity-45 " +
                b.className
              }
            >
              {b.icon}
