#!/usr/bin/env python3
"""
Build the catalog seed JSON from `Service Catalog.xlsx`.
...
CATALOG_DIR = os.path.normpath(
    os.path.join(REPO, "..", "4. Project Documents", "2. Design", "04. Service Catalog")
)

"""
THE WORKBOOK IS "Service Catalog.xlsx" NOW.

It was "Service Catalog v5.xlsx" and was renamed with the same content. The
version lived in the FILENAME, which is why this broke: the cut number is a
property of the catalog's history, not of the file Scott opens, and every
re-cut would have renamed the file and silently broken this script again.

The old name is kept as a fallback rather than deleted, so a checkout that
still has the pre-rename workbook keeps building instead of failing on a file
that is right there under its old name.
"""
XLSX_CANDIDATES = ("Service Catalog.xlsx", "Service Catalog v5.xlsx")
XLSX = next(
    (
        os.path.join(CATALOG_DIR, name)
        for name in XLSX_CANDIDATES
        if os.path.exists(os.path.join(CATALOG_DIR, name))
    ),
    os.path.join(CATALOG_DIR, XLSX_CANDIDATES[0]),
)
...
def main():
    ...
    catalog = {
        "_source": (
            "4. Project Documents/2. Design/04. Service Catalog/Service Catalog.xlsx — "
