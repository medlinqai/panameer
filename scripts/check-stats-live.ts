/*
  ── ⚠⚠⚠ SUPERSEDED — THIS SCRIPT COULD NOT RUN INSIDE THE SWEEP (`E606`) ──

  ⚠ IT IS KEPT ON DISK UNDER `E164` AND IS WIRED TO NOTHING. `check:stats-live`
  now runs `e2e-stats/stats-live.spec.ts` under
  `playwright.stats-live.config.ts`, which starts or reuses the harness's own
  server and tears it down afterwards.

  ⚠⚠ WHY IT HAD TO MOVE: this file assumed a server was already listening on
  **:3199**, a port nothing in the harness starts. Inside `npm run sweep` it
  therefore failed in about two seconds, every single run, WITHOUT EXECUTING A
  SINGLE ASSERTION — while the sweep line beside it looked like an ordinary red.
  ⚠⚠⚠ SO SIXTEEN LIVE ASSERTIONS WERE PROTECTING NOTHING, and the only reason
  they ever passed was that a human happened to have a server up.
  ⚠ **A GATE THAT CANNOT RUN INSIDE THE SWEEP IS NOT A GATE.**

  ⚠ THE ASSERTIONS THEMSELVES WERE FINE and moved across unchanged in
  substance. Nothing here was wrong except where it expected to find a server.
*/
// /**
